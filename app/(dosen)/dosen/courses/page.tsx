...
export default async function DosenCoursesPage() {
  ...
  // 3. Fetch courses taught by the lecturer from database including new academic columns
  const { data: coursesData } = await supabase
    .from('courses')
    .select('id, name, code, prodi, semester, kelas, tahun_akademik, created_at, weight_tugas, weight_uts, weight_uas')
      ...
      <header className="border-b border-neutral-200 bg-white px-6 py-4 flex items-center justify-between shadow-2xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-neutral-500 font-medium">Pengelolaan Mata Kuliah</span>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-6 md:p-8 space-y-8">
        ...
        <div className="flex flex-col gap-1.5 border-b border-neutral-200 pb-5">
          ...
          <p className="text-neutral-500 text-sm max-w-2xl">
            Tambahkan mata kuliah baru dengan struktur akademik lengkap, serta saring dan cari kelas aktif Anda secara instan.
          </p>
        </div>
        ...
        {/* Interactive Client-side filtering table */}
