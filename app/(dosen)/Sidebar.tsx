...
export default function Sidebar({ profile, onLogout }: SidebarProps) {
  ...
  useEffect(() => {
    setIsMounted(true)
    const stored = localStorage.getItem('sidebar-collapsed-dosen')
    if (stored !== null) {
  ...
  const toggleSidebar = () => {
    ...
    localStorage.setItem('sidebar-collapsed-dosen', String(nextState))
  }
      ...
      >
        ...
        <div className={`py-5 border-b border-neutral-100 flex items-center transition-all duration-300 ${isCollapsed ? 'px-3 justify-center' : 'px-6 gap-3'}`}>
          ...
          <div className={`transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'}`}>
            <span className="font-bold text-base tracking-tight text-neutral-900 block whitespace-nowrap">Portal Dosen</span>
            <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block whitespace-nowrap">Lecturer Area</span>
          </div>
        </div>
        ...
        <div className={`p-4 border-t border-neutral-200 bg-neutral-50/50 flex flex-col gap-3 transition-all duration-300 ${isCollapsed ? 'items-center px-2' : ''}`}>
          <div className={`flex flex-col min-w-0 px-2 transition-all duration-300 ${isCollapsed ? 'w-0 h-0 opacity-0 overflow-hidden p-0' : 'w-auto opacity-100'}`}>
